...
function row(piece, isShoppingCart) {
	...
	ret += td(render(csv[3]), renderCls(csv[3]), ' tooltip="简约"');
	ret += td(render(csv[4]), renderCls(csv[4]), ' tooltip="华丽"');
	ret += td(render(csv[5]), renderCls(csv[5]), ' tooltip="活泼"');
	ret += td(render(csv[6]), renderCls(csv[6]), ' tooltip="优雅"');
	ret += td(render(csv[7]), renderCls(csv[7]), ' tooltip="可爱"');
	ret += td(render(csv[8]), renderCls(csv[8]), ' tooltip="成熟"');
	ret += td(render(csv[9]), renderCls(csv[9]), ' tooltip="清纯"');
	ret += td(render(csv[10]), renderCls(csv[10]), ' tooltip="性感"');
	ret += td(render(csv[11]), renderCls(csv[11]), ' tooltip="清凉"');
	ret += td(render(csv[12]), renderCls(csv[12]), ' tooltip="保暖"');
